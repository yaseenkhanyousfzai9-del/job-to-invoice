...
Customer behaviour rows remain mostly **PENDING**. CUST-FOUNDATION-01, CUST-AUTH-01, and CUST-DOMAIN-01 are IMPLEMENTED. SUPABASE-DEV-RUNTIME-ROLE-03 verified `app_api_login` on the US development project. CUST-DB-01, **CUST-API-01**, **CUST-API-02**, **CUST-API-03**, **CUST-API-04**, **CUST-UI-01**, **CUST-UI-02**, and **CUST-UI-03** are VERIFIED. **CUST-UI-04** is **IMPLEMENTED / AWAITING PHYSICAL VERIFICATION**. **R-CUS-31** (jobs↔customer composite FK) is **VERIFIED** on development US (`0004_jobs.sql`). **R-CUS-PRE-05** jobs table/FK is in place; Jobs HTTP create remains CUST-JOB-01 (**IMPLEMENTED** for DB only; list-by-customer read is in CUST-API-03). S19 is **partially** complete: list/search/create (**R-CUS-17**) VERIFIED; customer detail + associated jobs read/display (**R-CUS-18** / CUST-API-03 + CUST-UI-03) VERIFIED for current read-only scope; edit API (**R-CUS-38** / CUST-API-04) VERIFIED; edit UI (**CUST-UI-04**) coded awaiting physical Android; archive / restore / delete / Create Job UX remain PENDING. S07 create path is VERIFIED; S07 archive restore (`R-CUS-16`) remains PENDING. Live owner OTP (QA01/QA02) remains unverified. Archive / delete remain PENDING. CUS01, CUS02, and S06 are not complete overall.
...
| R-CUS-04 | CUS01 | Editing customer details affects future drafts only | S07 edit | `PATCH /customers/{id}` updates live row only | none (PRD allowlist) | PATCH updates `customers` only; `app.documents` absent (no snapshot write possible) — memory + live US 2026-09-21 | VERIFIED |
...
| R-CUS-07 | CUS02 | Archive removes customer from new-job pickers | S06 picker, S19 | `POST /customers/{id}/archive` `{archived:true}`; `archived_at` set | none (PRD allowlist) | Archived id absent from picker; present in archived list | PENDING |
| R-CUS-08 | CUS02 | Archived customer's existing jobs and documents remain accessible | S19 detail, S08 | Archive does not cascade or hide jobs | none (PRD allowlist) | GET job/detail still works after archive | PENDING |
