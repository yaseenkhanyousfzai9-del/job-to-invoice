...
Customer behaviour rows remain mostly **PENDING**. CUST-FOUNDATION-01, CUST-AUTH-01, and CUST-DOMAIN-01 are IMPLEMENTED. SUPABASE-DEV-RUNTIME-ROLE-03 verified `app_api_login` on the US development project. CUST-DB-01, **CUST-API-01**, **CUST-API-02**, **CUST-API-03**, **CUST-UI-01**, and **CUST-UI-02** are VERIFIED. **R-CUS-31** (jobs↔customer composite FK) is **VERIFIED** on development US (`0004_jobs.sql`). **R-CUS-PRE-05** jobs table/FK is in place; Jobs HTTP create remains CUST-JOB-01 (**IMPLEMENTED** for DB only; list-by-customer read is in CUST-API-03). S19 is **partially** complete: list/search/create entry (**R-CUS-17**) VERIFIED; **customer detail + associated jobs API** (**R-CUS-18** / CUST-API-03) VERIFIED; Customer Detail UI (CUST-UI-03), archive/delete UX remain PENDING. S07 create path is VERIFIED; S07 archive restore (`R-CUS-16`) remains PENDING. Live owner OTP (QA01/QA02) remains unverified. PATCH / archive / delete remain PENDING. CUS01, CUS02, and S06 are not complete overall.
