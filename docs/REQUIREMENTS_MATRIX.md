...
Customer behaviour rows remain mostly **PENDING**. CUST-FOUNDATION-01, CUST-AUTH-01, and CUST-DOMAIN-01 are IMPLEMENTED. SUPABASE-DEV-RUNTIME-ROLE-03 verified `app_api_login` on the US development project. CUST-DB-01, **CUST-API-01**, **CUST-API-02**, **CUST-API-03**, **CUST-UI-01**, and **CUST-UI-02** are VERIFIED. **CUST-UI-03** is **IMPLEMENTED / AWAITING PHYSICAL VERIFICATION**. **R-CUS-31** (jobs↔customer composite FK) is **VERIFIED** on development US (`0004_jobs.sql`). **R-CUS-PRE-05** jobs table/FK is in place; Jobs HTTP create remains CUST-JOB-01 (**IMPLEMENTED** for DB only; list-by-customer read is in CUST-API-03). S19 is **partially** complete: list/search/create entry (**R-CUS-17**) VERIFIED; **customer detail + associated jobs API** (**R-CUS-35A/B** / CUST-API-03) VERIFIED; Customer Detail UI coded (CUST-UI-03) awaiting physical Android; archive/delete UX remain PENDING. S07 create path is VERIFIED; S07 archive restore (`R-CUS-16`) remains PENDING. Live owner OTP (QA01/QA02) remains unverified. PATCH / archive / delete remain PENDING. CUS01, CUS02, and S06 are not complete overall.
...
| R-CUS-18 | S19 DEC-CUST-001 DEC-CUST-006 | Customer detail shows associated jobs via customer GET plus jobs filter | S19 detail | `GET /v1/customers/{id}` and `GET /v1/jobs?customer_id=` | none (PRD allowlist) | API VERIFIED 2026-09-21; mobile Detail UI IMPLEMENTED awaiting physical Android | IMPLEMENTED |
...
| R-CUS-20 | UI04 | Customer list/detail/form have loading, empty, loaded, refresh failure, offline, access-expired | S07, S19, S06 | API errors 401/5xx; cache later | none (PRD allowlist) | List VERIFIED 2026-09-20; Detail loading/error/retry/empty-jobs coded 2026-09-21 (physical pending); form matrix still open | IMPLEMENTED |
...
| R-CUS-45 | AUTHZ01 SEC02 QA03 | Cross-tenant customer object references return generic 404; no data in API, storage, or logs | GET/PATCH/archive/DELETE, job bind | 404 generic | none (PRD allowlist) | GET customer + jobs filter: QA03 proven 2026-09-21 (CUST-API-03). PATCH/archive/DELETE still PENDING | IMPLEMENTED |
