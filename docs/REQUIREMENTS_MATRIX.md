...
Customer behaviour rows remain mostly **PENDING**. CUST-FOUNDATION-01, CUST-AUTH-01, and CUST-DOMAIN-01 are IMPLEMENTED. SUPABASE-DEV-RUNTIME-ROLE-03 verified `app_api_login` on the US development project. CUST-DB-01, **CUST-API-01**, **CUST-API-02**, **CUST-UI-01**, and **CUST-UI-02** are VERIFIED. **R-CUS-31** (jobs↔customer composite FK) is **VERIFIED** on development US (`0004_jobs.sql`). **R-CUS-PRE-05** jobs table/FK is in place; Jobs HTTP create/list remains CUST-JOB-01 (**IMPLEMENTED** for DB only). S19 is **partially** complete: list/search/create entry (**R-CUS-17**) is VERIFIED; customer detail, associated jobs API/UI, archive/delete UX remain PENDING. S07 create path is VERIFIED; S07 archive restore (`R-CUS-16`) remains PENDING. Live owner OTP (QA01/QA02) remains unverified. GET by id / PATCH / archive / delete remain PENDING. CUS01, CUS02, and S06 are not complete overall. **CUST-API-03** prerequisites (list API + jobs FK) are ready; do not start until authorized.
