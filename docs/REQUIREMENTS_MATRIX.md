...
Customer behaviour rows remain mostly **PENDING**. CUST-FOUNDATION-01, CUST-AUTH-01, and CUST-DOMAIN-01 are IMPLEMENTED. SUPABASE-DEV-RUNTIME-ROLE-03 verified `app_api_login` on the US development project. CUST-DB-01, **CUST-API-01**, **CUST-API-02**, **CUST-API-03**, **CUST-API-04**, **CUST-UI-01**, **CUST-UI-02**, **CUST-UI-03**, and **CUST-UI-04** are VERIFIED. **R-CUS-31** (jobs↔customer composite FK) is **VERIFIED** on development US (`0004_jobs.sql`). **R-CUS-PRE-05** jobs table/FK is in place; Jobs HTTP create remains CUST-JOB-01 (**IMPLEMENTED** for DB only; list-by-customer read is in CUST-API-03). S19 is **partially** complete — **VERIFIED:** create, list/search, detail, associated jobs read-only display, edit; **PENDING:** archive, restore, delete, Create Job. S07 create + edit paths are VERIFIED; S07 archive restore (`R-CUS-16`) remains PENDING. Live owner OTP (QA01/QA02) remains unverified. Archive / delete remain PENDING. CUS01, CUS02, and S06 are not complete overall.
