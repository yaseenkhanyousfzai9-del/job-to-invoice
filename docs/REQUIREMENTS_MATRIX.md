...
**Customer Module (Team A) = COMPLETE / FROZEN / INTEGRATION-READY.** **S07 Customer Form = VERIFIED.** **S19 Customers = VERIFIED.** Handoff contract: `docs/CUSTOMER_MODULE_CONTRACT.md`. Final merge checklist: `docs/CUSTOMER_HANDOFF_CHECKLIST.md`. Baseline manifest: `docs/CUSTOMER_BASELINE_MANIFEST.md`. Overlap Auth/S05/S06: `docs/TEAM_A_OVERLAP_NOTES.md` (**DO NOT EXTEND**). Customer behaviour rows remain **PENDING** only where Quote/snapshot/apply-to-draft or offline matrix is open. CUST-FOUNDATION-01, CUST-AUTH-01, and CUST-DOMAIN-01 are IMPLEMENTED. SUPABASE-DEV-RUNTIME-ROLE-03 verified `app_api_login` on the US development project. CUST-DB-01, **CUST-API-01**–**CUST-API-06**, and **CUST-UI-01**–**CUST-UI-06** are VERIFIED. **R-CUS-31** (jobs↔customer composite FK) is **VERIFIED** on development US (`0004_jobs.sql`). **Create Job API** / **S06** / **S05 Jobs-list API** are platform overlap (not Customer ownership); **S05 Jobs list UI** is **IMPLEMENTED** (awaiting physical). S08 not started. Live owner OTP (QA01/QA02) remains unverified. CUS01 apply-to-draft / published-snapshot evidence still need Quote/document slices.
...
| R-CUS-03 | CUS01 | A job chooses one approval contact (one customer record) | S06 | `jobs.customer_id` composite FK | `job_created` later, no PII | Memory + live US 2026-09-21 (`POST /v1/jobs`). **PHYSICALLY VERIFIED** Android 2026-09-22 (S06): Job binds selected Customer; appears on Customer Detail Jobs | VERIFIED |
...
| R-CUS-07 | CUS02 | Archive removes customer from new-job pickers | S06 picker, S19 | `POST /customers/{id}/archive` `{archived:true}`; `archived_at` set | none (PRD allowlist) | Memory + live US 2026-09-21; physical Android Detail Archive 2026-09-21 (Active omits / Archived includes). S06 picker: **AUTOMATED VERIFIED ONLY** (active-only request; archived excluded) — not physically exercised 2026-09-22 | VERIFIED |
...
| ID | PRD | Requirement | Screen/Flow | Backend | Analytics | Tests | Status |
|---|---|---|---|---|---|---|---|
| R-CUS-12 | S06 | Create Job selects a customer; can open customer creation sheet | S06 | `GET /customers`, `POST /customers`, `POST /jobs` | `job_created` when job exists; no customer PII | Create Job API VERIFIED 2026-09-21. **PHYSICALLY VERIFIED** Android 2026-09-22 (S06): picker select/search; Add customer return; no-site/site/Quote/Direct invoice creates; network Retry; success → Customer Detail | VERIFIED |
...
| R-CUS-21 | UI01 UI02 UI03 NFR01 | Customer UI uses specified tokens, 44×44 targets, Dynamic Type, VoiceOver labels, light appearance | S07, S19, S06 | none | none (PRD allowlist) | Targets + labels + archived text + destructive Delete separation audited 2026-09-22 (`customerAccessibility.test.ts`); full Dynamic Type / VoiceOver physical QA still open | IMPLEMENTED |
...
| R-CUS-PRE-05 | POST /jobs DB02 jobs | Minimum jobs table and create/list so Customer picker, associated jobs, and referenced-delete are real | S06, S19 | `jobs.customer_id` | `job_created` | Table+FK+RLS VERIFIED; list-by-customer VERIFIED (CUST-API-03); **Create Job API VERIFIED** 2026-09-21; **S06 mobile UI VERIFIED** physical Android 2026-09-22; **S05 Jobs-list API VERIFIED** 2026-09-22 (DEC-JOB-001); **S05 Jobs UI IMPLEMENTED** (awaiting physical). S08 not claimed | VERIFIED |
