...
Customer behaviour rows remain mostly **PENDING**. CUST-FOUNDATION-01, CUST-AUTH-01, and CUST-DOMAIN-01 are IMPLEMENTED. SUPABASE-DEV-RUNTIME-ROLE-03 verified `app_api_login` on the US development project. CUST-DB-01, **CUST-API-01**, **CUST-API-02**, **CUST-API-03**, **CUST-API-04**, **CUST-API-05**, **CUST-UI-01**, **CUST-UI-02**, **CUST-UI-03**, and **CUST-UI-04** are VERIFIED. **R-CUS-31** (jobs↔customer composite FK) is **VERIFIED** on development US (`0004_jobs.sql`). **R-CUS-PRE-05** jobs table/FK is in place; Jobs HTTP create remains CUST-JOB-01 (**IMPLEMENTED** for DB only; list-by-customer read is in CUST-API-03). S19 is **partially** complete — **VERIFIED:** create, list/search, detail, associated jobs read-only display, edit, archive/restore API; **PENDING:** archive/restore UI, delete, Create Job. S07 create + edit paths are VERIFIED; S07 archive restore UI (`R-CUS-16` / CUST-UI-05) remains PENDING. Live owner OTP (QA01/QA02) remains unverified. Delete remains PENDING. CUS01, CUS02, and S06 are not complete overall.
...
| R-CUS-07 | CUS02 | Archive removes customer from new-job pickers | S06 picker, S19 | `POST /customers/{id}/archive` `{archived:true}`; `archived_at` set | none (PRD allowlist) | Memory + live US 2026-09-21: archive succeeds; Active list omits; Archived includes. Picker UX still CUST-JOB-01 / CUST-UI-05 | IMPLEMENTED |
...
| R-CUS-16 | S07 | Archived customer restore option on the form | S07 | `POST /customers/{id}/archive` `{archived:false}` | none (PRD allowlist) | Restore API VERIFIED 2026-09-21 (CUST-API-05); form restore control is CUST-UI-05 | PENDING |
