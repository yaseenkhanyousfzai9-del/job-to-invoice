...
Customer behaviour rows remain mostly **PENDING**. CUST-FOUNDATION-01, CUST-AUTH-01, and CUST-DOMAIN-01 are IMPLEMENTED. SUPABASE-DEV-RUNTIME-ROLE-03 verified `app_api_login` on the US development project. CUST-DB-01, **CUST-API-01**–**CUST-API-06**, and **CUST-UI-01**–**CUST-UI-06** are VERIFIED. **R-CUS-31** (jobs↔customer composite FK) is **VERIFIED** on development US (`0004_jobs.sql`). **R-CUS-PRE-05** jobs table/FK is in place; **Create Job API** (`POST /v1/jobs`) is **VERIFIED** (memory + live US 2026-09-21). **S06 / Create Job + Customer picker UI** is **VERIFIED** (physical Android 2026-09-22). **S05 Jobs-list API** (`GET /v1/jobs` general + `bucket`, DEC-JOB-001) is **VERIFIED** (memory + live US 2026-09-22). **S05 Jobs list UI** is **IMPLEMENTED** (awaiting physical Android verification 2026-09-22). S08 Job Detail is not started. **S19** (Customers tab) is **VERIFIED** (2026-09-21). S07 create + edit + archive/restore Detail paths are VERIFIED. Job overview (S08) remains unimplemented. Live owner OTP (QA01/QA02) remains unverified. CUS01 apply-to-draft and published-snapshot evidence still need Quote/document slices.
