...
Customer behaviour rows remain mostly **PENDING**. CUST-FOUNDATION-01, CUST-AUTH-01, and CUST-DOMAIN-01 are IMPLEMENTED. SUPABASE-DEV-RUNTIME-ROLE-03 verified `app_api_login` on the US development project. CUST-DB-01, **CUST-API-01**, **CUST-API-02**, **CUST-API-03**, **CUST-API-04**, **CUST-API-05**, **CUST-UI-01**, **CUST-UI-02**, **CUST-UI-03**, and **CUST-UI-04** are VERIFIED. **CUST-UI-05** is **IMPLEMENTED / AWAITING PHYSICAL VERIFICATION**. **R-CUS-31** (jobs↔customer composite FK) is **VERIFIED** on development US (`0004_jobs.sql`). **R-CUS-PRE-05** jobs table/FK is in place; Jobs HTTP create remains CUST-JOB-01 (**IMPLEMENTED** for DB only; list-by-customer read is in CUST-API-03). S19 is **partially** complete — **VERIFIED:** create, list/search, detail, associated jobs read-only display, edit, archive/restore API; **PENDING:** archive/restore UI physical Android, delete, Create Job. S07 create + edit paths are VERIFIED; S07 archive restore UI (`R-CUS-16` / CUST-UI-05) is coded awaiting physical Android. Live owner OTP (QA01/QA02) remains unverified. Delete remains PENDING. CUS01, CUS02, and S06 are not complete overall.
...
| R-CUS-07 | CUS02 | Archive removes customer from new-job pickers | S06 picker, S19 | `POST /customers/{id}/archive` `{archived:true}`; `archived_at` set | none (PRD allowlist) | Memory + live US 2026-09-21: archive succeeds; Active list omits; Archived includes. Detail Archive UI coded (CUST-UI-05). Picker UX still CUST-JOB-01 | IMPLEMENTED |
| R-CUS-08 | CUS02 | Archived customer's existing jobs and documents remain accessible | S19 detail, S08 | Archive does not cascade or hide jobs | none (PRD allowlist) | Memory + live US 2026-09-21: archive with jobs; `GET /jobs?customer_id=` and detail still work; restore preserves jobs | VERIFIED |
| R-CUS-09 | CUS02 | Unreferenced customers may be deleted | S19 | `DELETE /customers/{id}` | none (PRD allowlist) | Unreferenced DELETE removes row | PENDING |
| R-CUS-10 | CUS02 DEC-CUST-004 | Referenced customers are not destructively deleted; offer archive; workspace export is EXP01 when built (no customer-scoped export) | S19 | `DELETE` → 409 `CUSTOMER_REFERENCED`; FK prevents DB delete | none (PRD allowlist) | Referenced DELETE 409; row remains; no fake export | PENDING |
...
| R-CUS-16 | S07 | Archived customer restore option on the form | S07 | `POST /customers/{id}/archive` `{archived:false}` | none (PRD allowlist) | Restore API VERIFIED 2026-09-21 (CUST-API-05); Detail Restore UI coded (CUST-UI-05) awaiting physical Android | IMPLEMENTED |
| R-CUS-17 | S19 | Customers tab: search, list, create | S19 | `GET /customers` search/page | none (PRD allowlist) | Physical Android 2026-09-20: route, Active/Archived/All, search, create entry, return-after-create refresh. S19 edit VERIFIED via CUST-UI-04 2026-09-21; archive/restore/delete/Create Job still PENDING | VERIFIED |
| R-CUS-18 | S19 DEC-CUST-001 DEC-CUST-006 | Customer detail shows associated jobs via customer GET plus jobs filter | S19 detail | `GET /v1/customers/{id}` and `GET /v1/jobs?customer_id=` | none (PRD allowlist) | API VERIFIED 2026-09-21; physical Android 2026-09-21: detail nav, name/email/address, empty jobs, Back, search preservation, API-off error + Retry recovery (session kept); edit entry + post-edit detail refresh physical Android 2026-09-21 (CUST-UI-04); populated jobs/pagination/404/archived badge automated only | VERIFIED |
...
| R-CUS-45 | AUTHZ01 SEC02 QA03 | Cross-tenant customer object references return generic 404; no data in API, storage, or logs | GET/PATCH/archive/DELETE, job bind | 404 generic | none (PRD allowlist) | GET customer + jobs filter: QA03 proven 2026-09-21 (CUST-API-03). PATCH cross-tenant identical 404 proven 2026-09-21 (CUST-API-04). Archive/restore cross-tenant identical 404 proven 2026-09-21 (CUST-API-05). DELETE still PENDING | IMPLEMENTED |
