...
Customer behaviour rows remain mostly **PENDING**. CUST-FOUNDATION-01, CUST-AUTH-01, and CUST-DOMAIN-01 are IMPLEMENTED. SUPABASE-DEV-RUNTIME-ROLE-03 verified `app_api_login` on the US development project. CUST-DB-01, **CUST-API-01**, **CUST-API-02**, **CUST-API-03**, **CUST-API-04**, **CUST-API-05**, **CUST-UI-01**, **CUST-UI-02**, **CUST-UI-03**, **CUST-UI-04**, and **CUST-UI-05** are VERIFIED. **R-CUS-31** (jobs↔customer composite FK) is **VERIFIED** on development US (`0004_jobs.sql`). **R-CUS-PRE-05** jobs table/FK is in place; Jobs HTTP create remains CUST-JOB-01 (**IMPLEMENTED** for DB only; list-by-customer read is in CUST-API-03). S19 is **partially** complete — **VERIFIED:** create, list/search, detail, associated jobs read-only display, edit, archive, restore; **PENDING:** delete, Create Job. S07 create + edit + archive/restore Detail paths are VERIFIED. Live owner OTP (QA01/QA02) remains unverified. Delete remains PENDING. CUS01, CUS02, and S06 are not complete overall.
...
| R-CUS-17 | S19 | Customers tab: search, list, create | S19 | `GET /customers` search/page | none (PRD allowlist) | Physical Android 2026-09-20: route, Active/Archived/All, search, create. S19 edit VERIFIED CUST-UI-04; archive/restore VERIFIED CUST-UI-05 2026-09-21; delete/Create Job still PENDING | VERIFIED |
